...
async function runBuild(job, ctx) {
  ...
  if (ctx.uploadZip && ctx.zipKey) {
    ...
    try {
      ...
      // Не роняем всю сборку: архив есть, просто поедет медленным путём.
...
function ensureBuild(ctx, opts) {
  ...
  if (fs.existsSync(cachePath)) {
    ...
    // Локальная копия без метки — значит выгрузка в хранилище не удалась.
    // Отдаём запасным путём, через сам сервер.
