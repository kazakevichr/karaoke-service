...
function jobView(job) {
  ...
  return {
    state: job.state,          // building | uploading | ready | error
    done: job.done,
    total: job.total,
    bytes: job.bytes,
...
/* Путь к «метке» рядом с архивом: в ней лежит ссылка на объект в S3, если
   архив туда уже выгружен. Нужна, чтобы после перезапуска сервера не гадать,
   доехал архив до хранилища или нет — реестр сборок живёт в памяти и
   рестарта не переживает, а метка на диске переживает. */
const urlMarkerPath = cachePath => `${cachePath}.url`;
...
async function runBuild(job, ctx) {
  ...
  /* ---- Выгрузка архива в S3 ----
     Отдавать почти гигабайтный файл через сам сервер приложения оказалось
     тупиком: канал «сервер → пользователь» выдаёт около 60 КБ/с, то есть
     904 МБ качались бы больше четырёх часов. При этом канал «сервер ↔ S3»
     быстрый (сборка выкачивает те же ~900 МБ за разумное время), и хранилище
     отдаёт файлы пользователю быстро — на нём и так работает всё аудио в
     онлайн-игре. Поэтому готовый архив кладём в S3 и даём прямую ссылку:
     скачивание идёт мимо узкого места и заодно не занимает собой маленький
     VPS на всё время раздачи.

     Локальную копию после успешной выгрузки удаляем — иначе каждая
     сохранённая игра занимала бы на диске сервера почти гигабайт вдобавок к
     тому же гигабайту в хранилище. Если выгрузка не удалась, копия остаётся
     и работает запасной путь через сам сервер (медленный, но рабочий). */
  await uploadStep(job, ctx);
}

/* Выгрузка готового файла в хранилище. Вынесена отдельно, потому что нужна в
   двух случаях: сразу после сборки и для архивов, собранных ещё до появления
   выгрузки — их нужно просто перенести, пересобирать нечего. */
async function uploadStep(job, ctx) {
  const { cachePath, log } = ctx;
  if (!ctx.uploadZip || !ctx.zipKey) return;

  job.state = 'uploading';
  job.uploaded = 0;
  if (!job.bytes) job.bytes = fs.statSync(cachePath).size;
  try {
    const url = await ctx.uploadZip(cachePath, ctx.zipKey, loaded => { job.uploaded = loaded; });
    await fsp.writeFile(urlMarkerPath(cachePath), url, 'utf8');
    await fsp.unlink(cachePath).catch(() => {});
    job.url = url;
    log('архив выгружен в хранилище:', url);
  } catch (e) {
    // Не роняем всё: архив есть, просто поедет медленным путём через сервер.
    log('выгрузка в хранилище не удалась, отдаём через сервер:', e.message);
  }
}
...
function ensureBuild(ctx, opts) {
  ...
  // Архив уже в хранилище — отдаём прямую ссылку, ничего пересобирать не надо.
  // Метка на диске переживает перезапуск сервера, в отличие от реестра сборок.
  const marker = urlMarkerPath(cachePath);
  if (fs.existsSync(marker)) {
    return { state: 'ready', done: 1, total: 1, bytes: 0, uploaded: 0, failed: 0, error: null,
      url: fs.readFileSync(marker, 'utf8').trim() };
  }

  /* Текущую сборку проверяем РАНЬШЕ файла кэша. Порядок здесь принципиален:
     файл появляется на своём месте сразу после сборки, но до выгрузки в
     хранилище. Если сначала смотреть на файл, то на всё время выгрузки клиент
     получал бы «готово» с пустой ссылкой — то есть качал бы медленным путём
     через сервер именно тогда, когда быстрый вот-вот появится. */
  const running = jobs.get(id);
  if (running && (running.state === 'building' || running.state === 'uploading')) return jobView(running);

  /* Локальная копия есть, а метки нет: либо архив собран ещё до того, как
     появилась выгрузка в хранилище, либо она в прошлый раз не удалась. В
     обоих случаях пересобирать нечего — файл готов, его надо просто отнести
     в хранилище. Без этой ветки уже собранные архивы навсегда остались бы на
     медленном пути через сервер, и единственным лекарством было бы удалить
     игру и собрать её заново — то есть выкачать те же ~900 МБ из S3 второй
     раз безо всякой нужды. */
  if (fs.existsSync(cachePath)) {
    const existing = jobs.get(id);
    if (existing && existing.state === 'ready') return jobView(existing);

    if (ctx.uploadZip && ctx.zipKey) {
      const job = { state: 'uploading', done: 1, total: 1, bytes: 0, uploaded: 0, url: null,
        failed: [], error: null, startedAt: Date.now() };
      jobs.set(id, job);
      uploadStep(job, ctx).finally(() => { job.state = 'ready'; });
      return jobView(job);
    }
    // Хранилище не настроено — отдаём запасным путём, через сам сервер.
    return { state: 'ready', done: 1, total: 1, bytes: fs.statSync(cachePath).size, uploaded: 0, failed: 0, error: null, url: null };
  }
  ...
  const job = { state: 'building', done: 0, total: 0, bytes: 0, uploaded: 0, url: null, failed: [], error: null, startedAt: Date.now() };
  jobs.set(id, job);
